"""Novel in-context task benchmark."""
...
class ICLNovelTaskTest(BaseTest):
    """Train on affine ICL prompts and evaluate on held-out modulo rules."""

    PAD = 0
    SEP = 241
    QUERY = 242

    def __init__(self, config: Dict[str, Any], name: str = "ICLNovelTaskTest"):
    ...
    def _rule(self, x: int, task_id: int, eval_split: bool) -> int:
        if eval_split:
            return ((x * x) + 3 * task_id + 7) % 96 + 20
        return ((task_id + 2) * x + 5 * task_id) % 96 + 20

    def _make_dataset(self, count: int, offset: int, eval_split: bool, device) -> Tuple[torch.Tensor, torch.Tensor]:
        generator = torch.Generator(device="cpu")
        generator.manual_seed(int(self.random_seed) + int(offset))
        xs, ys = [], []
        for _ in range(count):
            task_id = int(torch.randint(0, 6, (1,), generator=generator).item())
            seq = [30 + task_id]
            used = set()
            for _shot in range(self.num_shots):
                x_val = int(torch.randint(0, 24, (1,), generator=generator).item())
                used.add(x_val)
                seq.extend([x_val + 20, self._rule(x_val, task_id, eval_split), self.SEP])
            query = int(torch.randint(0, 24, (1,), generator=generator).item())
            while query in used:
                query = int(torch.randint(0, 24, (1,), generator=generator).item())
            seq.extend([self.QUERY, query + 20])
            target = self._rule(query, task_id, eval_split)
    ...
    def prepare_data(self) -> Tuple[torch.Tensor, torch.Tensor]:
        return self._make_dataset(self.num_samples, 0, False, self.device)
    ...
    def run(self, model: nn.Module) -> Dict[str, Any]:
        ...
        train_x, train_y = self._make_dataset(self.num_samples, 0, False, device)
        eval_x, eval_y = self._make_dataset(self.num_eval_samples, 10_000, True, device)
