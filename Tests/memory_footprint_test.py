...
class MemoryFootprintTest(BaseTest):
    ...
    def run(self, model: nn.Module) -> Dict[str, Any]:
        ...
        largest_output_mb = per_length.get(str(largest), {}).get("output_memory_mb", 0.0)
        memory_efficiency = largest / max(1.0, parameter_memory_mb + largest_output_mb)
        result = {
            ...
            "inference_speed": (self.batch_size * sum(self.lengths)) / total_time if total_time > 0 else 0,
