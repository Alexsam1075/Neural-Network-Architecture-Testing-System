"""Reverse sequence copying benchmark."""
...
class ReverseCopyTest(BaseTest):
    """Read a random payload and reproduce it in reverse order."""

    PAD = 0
    SEP = 241

    def __init__(self, config: Dict[str, Any], name: str = "ReverseCopyTest"):
    ...
    def _make_dataset(self, count: int, offset: int, device) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for _ in range(count):
            ...
            y[: self.copy_length] = torch.flip(payload, dims=[0])
            inputs.append(x)
            targets.append(y)
    ...
    def _evaluate(self, model: nn.Module, x: torch.Tensor, y: torch.Tensor) -> Dict[str, float]:
    ...
    def run(self, model: nn.Module) -> Dict[str, Any]:
        ...
        metrics = self._evaluate(model, eval_x, eval_y)
